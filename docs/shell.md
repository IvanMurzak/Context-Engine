...
**The popup rect is the ONE geometry CEF hands over in DIP** (`a2`). `OnPopupSize` reports "the new
location and size in view coordinates" while the popup's own `OnPaint` buffer is physical like every
other frame, so the compositor converts: the **origin** is scaled by the monitor factor and the
**size** is taken from the texture, never from the scaled DIP size (CEF paints `ceil(DIP x scale)`,
which `round(DIP x scale)` does not always reproduce). Used raw — as it was until `a2` — a 150 %
`<select>` drew two thirds of the way toward the window origin at two thirds the size (cropped on the
CPU path, resampled on the GPU one) while CEF hit-tested the true DIP rect, so a click on the drawn
menu landed outside it and dismissed it. Arrow keys and Enter worked throughout, which is the tell.

That conversion needs a scale, and **`WindowCompositor::dpi_` is the compositor's one DPI seam**: it
rides `on_resize(physical_size, DpiScale)` the way `IBrowserHost::resize` already carries it, is
pushed from `EditorWindow::sync_browser_size` and nowhere else, and is what `e3` takes when it needs
a scale for viewport rects. A second DPI source beside it is the failure mode the seam was chosen to
avoid. The conversion deliberately does NOT happen in the CEF binding, for the reason `dpi.h` states
about the screen mapping: that TU is the one the local gate cannot build and no headless CI job
executes, whereas the compositor is CEF-free and runs on all three `build` legs.
...
`InputTarget::viewport` pointer arm now queues a PRESS for picking (§ 4c) — orbit/pan/zoom gesture
math is still later work.

## 4c. Viewport picking (M9 editor-UX e4, D8)

A click resolves to an entity via a **CPU raycast**, never a GPU id-buffer: assertable on all three
`build` legs with no GPU, this repo's headless-first rule (accepted cost: worse accuracy at geometry
silhouettes). `context/render/picking.h`'s `pick_nearest(ray, snapshot, proxy_size)` is the pure
core — it raycasts each drawable's box PROXY (`viewport_pass.h`'s `proxy_model`, EXPORTED so picking
tests against the EXACT box the pass draws, never a second bounding-volume convention free to drift),
tested in the box's own OBJECT space via its inverse model matrix so rotation and non-uniform scale
are both honoured, and compares candidates by real WORLD distance (an object-space parametric `t` is
not comparable across differently-scaled boxes).

**The wiring, split across the D10 boundary the same way the producer is.** `EditorWindow`'s
`InputTarget::viewport` arm (core Shell, `context_editor_shell`) queues a `ViewportPressEvent`
(region id, region-relative point, region size) on a primary-button PRESS only — hovers, releases and
non-primary buttons are dropped, because picking fires once per press, not once per pointer sample —
drained per frame by `take_viewport_presses()`, the SAME queue idiom as `ViewportBinding::take_dirty`.
The composition root (`editor_main.cpp`, which links the panel libraries the core Shell cannot) hands
each press to `panels::ViewportFeed::pick()`: it reads the copy's OWN camera
(`ViewportBinding::camera`, never a fixed transform — a moved camera picks a different entity),
builds the ray through `render::pick_ray`, raycasts it against the Shell's `viewport_scene()`, and
issues the result as the Scene tree's OWN write — `SceneTreePanel::select()` on a hit,
`clear_selection()` on a miss ("an empty replace"). **No new channel:** `select()` writes through the
e08b gateway and renders the daemon's reply itself, so the Inspector's existing selection listener
fires exactly as it would for a tree-row click — propagation to Hierarchy and Inspector is free.

**The entity id is a deliberately TEMPORARY encoding.** `RenderSnapshot`'s `kernel::Entity` has no
real L-35 composed-identity path yet (§ above: the e11c scene-data verb was never built), so
`pick_selection_id()` stringifies the entity HANDLE itself (`entity:<index>:<generation>`) — stable,
opaque to the daemon, and the one call site that changes the day a real composed-identity read lands.
Combined with the still-EMPTY production `RenderSnapshot`, a live pick always misses today; that is
the honest state the whole producer is in, not a defect in the picker, and the T1/T2 suites prove the
algorithm and the wiring against CONSTRUCTED snapshots.
...
- **The two members do NOT share a convention** (pinned `cef_render_handler.h`): `GetScreenPoint`
  wants screen **device pixels** on Windows/Linux and screen **DIP** on macOS, while
  `GetRootScreenRect` is **DIP on every platform**. Applying the split to both multiplies the root
  rect by the scale factor on Windows/Linux, and like every bug in this family it is invisible at
  100 %. `GetScreenPoint` is now the **only** member that takes the split: `a2` established that
  `GetScreenInfo::rect` is DIP too (see § the popup rect below), so the file-scope
  `kScreenCoordsAreDip` has two readers, not three.
- **The arithmetic is in `dpi.h`, not in the CEF binding** (`osr_screen_point` /
  `osr_root_screen_rect`, and `a2`'s `osr_popup_dest_rect`): the per-platform branch is the ONE
  branch the local gate cannot build *and* no CI job executes. The binding passes its platform's
  convention as a single file-scope constant that both screen callbacks share, so the two cannot
  disagree.
...
| `editor-shell-test_compositor` | The extrapolated layer UV (incl. the full-window identity vs e03), the premultiplied blend + clipping, damage-driven skip, LAYER ORDER and the popup's scissor rect, a hidden popup dropping its layer, the resize protocol **and the scale that rides it**, Outdated/Lost keeping the damage, Suboptimal presenting first, a refused surface, both present paths, a malformed producer frame; **a2**: the popup's DIP -> physical destination on BOTH present paths at scale 1.5 (pixels asserted inside AND outside the rect, and the DIP rect asserted to be a different rect on all four numbers), the destination SIZE coming from the texture rather than the scaled DIP size at 1.25 (where `ceil` and `round` part), and a DPI change moving an already-open popup. The 1.0 cases are kept beside them: at 1.0 the correct and broken code are byte-identical, which is why this bug shipped green |
| `editor-shell-test_shell` | The attach guard, the owner loop end to end (DIP browser sizing, input round-trip, viewport vs browser, focus dropping a live drag, idle skip, popup), placement persistence + restore, window drop, shutdown flush; **a1**: the client origin seeded before the first paint and updated on the `moved` event under the REAL 250 ms poll interval (so the push is attributable to the event, not to the poll), the modelled frameless inset proving it is the CLIENT origin and not the window rect, the placement-poll BACKSTOP for a move with no event, the COALESCING (four moves drained in one pump push ONCE, carrying the last position — the per-event form scored five), and the negative half — an idle pump pushes nothing; **a2**: a popup composited at its PHYSICAL rect on a 150 % monitor end to end — the one test that reddens if the compositor's scale is never pushed, which every compositor-level test (they set the scale by hand) would miss; **e4**: a primary-button PRESS inside a live viewport region queued for picking (region id, region-relative point, the published rect's size), drained by `take_viewport_presses()`, with the negative half — a hover and a non-primary press queue nothing |
...
| `editor-shell-test_viewport_feed` | editor-UX e3 / c3: the tree's ONE `provide_factory` binding — the bind-time probe materialising NO model, two copies with two models and two cameras (framing one leaves the other's generation and camera untouched), the `viewport.frame-scene` → `editor.camera-set` → `editor.cameras-get` round trip through the OPAQUE payload, a failed write RE-ARMED without changing what it retries, a new copy re-arming the hydration read, `persists: false` (the camera lives where cameras live), and the `viewport.adapter_absent` summary WITH its negative half; **e4** (D8), driven against a REAL `SessionFeed` over a scripted wire channel: a hit moves a REAL Scene-tree row (non-zero hash) and fires its selection listener (the Inspector's own trigger) while the Files panel stays untouched (both directions), a miss issues an EMPTY `editor.select` ("an empty replace") after first landing a real selection so the clear is a genuine change, the SAME click resolving to a DIFFERENT entity purely because the copy's own camera moved, and the honest no-op with no Scene tree bound |
...
| `PET_POPUP` | Open a `<select>` / dropdown: the popup renders over the page, clipped to its own rect, and closes cleanly with no ghost of it left behind. **On a non-100 % monitor** the menu must appear under the control it belongs to, at full size — the `a2` symptoms. `a2` closed the placement half: the DIP→physical destination on both present paths, at scale 1.5 and at 1.25 (where `ceil` and `round` part on the destination SIZE), is pinned by `editor-shell-test_compositor`; the same suite's `test_a_hidden_popup_drops_its_layer` pins the ghost-free hide; and `editor-shell-test_shell`'s `test_a_popup_composites_at_the_physical_rect_on_a_scaled_monitor` pins the whole loop end to end on a scaled monitor. What stays manual (`f2`): that a CLICK on the rendered popup actually selects — CEF resolves that hit internally once it is handed the right numbers, and no smoke drives an actual dropdown-open-then-click gesture into a live browser to observe the outcome. |
...
| Context menu placement (a1) | Move the window well away from the screen origin, then right-click inside it: CEF's own menu opens **at the cursor**, not offset by the window's position. Repeat at 150 % scaling and with the window MAXIMIZED (the state a placement-derived origin would get wrong), and on a second monitor placed LEFT of the primary one, where the client origin is negative. `editor-shell-test_dpi` pins exactly these three scenarios against the pure arithmetic (`osr_screen_point` / `osr_root_screen_rect`), and `editor-shell-test_shell` pins the push plumbing (`client_origin()` → `set_client_origin()`) that feeds it — both on all three `build` legs (§ 8). The callback only ever runs against a REAL window on Linux, whose CEF smokes open a genuine X11 window (§ 8's `editor-cef-smoke-shell`); Windows and macOS keep the offscreen backend for their live CEF smokes too (§ 10), so this path is not exercised live there at all. No smoke on any OS drives an actual right-click, so what only a real desktop can show is that the on-screen menu lands at the pixel the arithmetic computed. |
...
| `OnAcceleratedPaint` | deliberately not needed | Owner ruling 2026-07-19: stock wgpu-native exposes no external-texture import and a patched fork was rejected; rationale recorded in `cef_shell.h:14-19`. The seam stays wired (`CefShellOptions::accelerated_osr` feeds e03's `OsrImportOptions`) and `shared_texture_enabled` is left at its default (off), so CEF never calls this. |
