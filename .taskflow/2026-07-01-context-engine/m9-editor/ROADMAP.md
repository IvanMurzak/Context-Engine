...
| **editor-window-chrome → e16 HAND-OFF: the finished window chrome is a STABLE surface for the visual-regression harness to pin** (recorded by g1, 2026-08-29; set `.taskflow/2026-08-28-editor-window-chrome/`, PRs #480–#488 + g1) | The chrome set (02 §12) deliberately ships NO visual-regression CI — that is e16's `editor-visual-regression` job — and hands e16 a surface that will not move under it: the four-strip frame `#editor-titlebar` (38 px) / `#editor-playbar` (40 px) / `#editor-root` / `#editor-statusbar` (24 px) with `#editor-banners` the fixed overlay (`app/index.html`, heights as `:root` custom properties in `app.css`); the three chrome modes gated off `chrome.state.mode` and observable as `data-chrome-mode` (`custom` = Windows: brand + web menubar + title + the `data-chrome-control` minimize/maximize/close cluster; `hybrid` = macOS: no cluster, left-padded by the measured `controlsInset`; `system` = Linux: menu-bar-only strip), the window role as `data-chrome-window` (`primary` / `secondary` — the compact secondary strip has NO play bar / statusbar DOM), the max/restore glyph as `data-maximized`; the `<html>` boot reports `data-editor-strips` / `data-editor-playbar` / `data-editor-statusbar`; the play bar's `data-play-state` (`idle` / `running` / `paused` reachable; `compiling` / `error` declared, unreachable) driving the `.ctx-flourish`; the web menubar (`.ctx-menubar`, ARIA `menubar`/`menu`/`menuitem`, 03's tree — absent on macOS where the native `NSMenu` renders it); the statusbar segments (daemon link / problems count / theme / project); the region ids `chrome.caption` / `chrome.caption-min` / `-max` / `-close`. Pixel authority is the d1 mockup (`mockups/editor.html`, `shared/components.css` `.titlebar`/`.play-bar`/`.statusbar`), both themes. What e16 must NOT expect: FPS in the play bar (no source until e11), a live Target chip (static "Scene", disabled), Alt-mnemonics on the menubar, CSD on Linux (D6), and the ten CEF smokes windowed on macOS (CE #443) — `mode:"hybrid"` is provable only in the CEF-free `editor-shell-cocoa-window`. Everything CI already pins for the chrome (the g1 verification map + the per-OS still-manual rows) is in CE `docs/shell.md` § 15 / § 10. | **e16** (`editor-visual-regression`): golden per panel × {Dark, Light} now has a stable frame to include — pin the strips in BOTH welcome and project modes and BOTH window roles; the deliberate-pixel-change-fails-until-re-blessed DoD applies to the chrome too. |
