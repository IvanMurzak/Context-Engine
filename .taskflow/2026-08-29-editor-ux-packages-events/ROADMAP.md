...
**`e3` MEETS its DoD while two user-visible gaps remain — the distinction is deliberate, not a fudge.**
The DoD asks for `camera-set` → `cameras-get` → *persisted and restored*, a DATA round trip, and that
holds. It does **not** ask for the pixels to update — so D1 (a camera move never re-renders; the pose
persists to `.editor/session.json` while the composited image keeps the old one) is outside the DoD as
written while still being incomplete to a user. Likewise the instance proof holds WITHIN a host, while
D2 (both windows minting `builtin.viewport#1`, because the ordinal is per `PanelHost`) bites ACROSS
windows, which the DoD never states. Both are registered above rather than counted as delivered.
