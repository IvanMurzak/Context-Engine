...
GOOD_HEADERS = {
    ...
    "context/editor/bridge/transport.h": '#include "context/editor/contract/json.h"\n',
    "context/kernel/event_bus.h": "#include <cstdint>\n",
}


def run(prefix: Path, consumer: Path | None = None) -> int:
...
def test_include_resolving_to_no_installed_header_fails(tmp_path: Path) -> None:
    ...
    headers = dict(GOOD_HEADERS)
    headers["context/editor/client/client.h"] = '#include "context/editor/bridge/not_shipped.h"\n'
    prefix = make_install(tmp_path, headers)
    assert run(prefix) == 1
...
def test_transitive_leak_is_caught(tmp_path: Path) -> None:
    ...
    headers = dict(GOOD_HEADERS)
    headers["context/editor/client/client.h"] = '#include "context/editor/bridge/transport.h"\n'
    headers["context/editor/bridge/transport.h"] = (
...
def test_project_includes_handles_angle_bracket_project_headers() -> None:
    assert check_include_graph.project_includes("#include <context/kernel/event_bus.h>\n") == [
        "context/kernel/event_bus.h"
    ]
...
def test_is_allowed_module() -> None:
    ...
    assert not check_include_graph.is_allowed_module("context/kernel")
