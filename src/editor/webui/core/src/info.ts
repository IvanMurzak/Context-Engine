// @context-engine/editor-core — the contract-surface projection (design 04 section 1).
//
// e05a landed this as the workspace's whole entry point, proving the toolchain chain end to end
// (pinned esbuild bundle + pinned tsgo typecheck + registry-generated client typings). e05c split
// it out of `index.ts` unchanged: `index.ts` is now the APP entry (it boots the bridge), and this
// file keeps its original job — the typed, generated-schema-backed view of the contract surface.
//
// Keep it that way. The bridge is `bridge.ts`, the boot handshake is `boot.ts`, and PanelHost /
// the hydration runtime are e05d's. Nothing app-shaped belongs here.
