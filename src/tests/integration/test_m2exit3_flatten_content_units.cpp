...
int main()
{
    ...
    // CHECK() logs and continues (m2_exit_test.h); guard the size- and pointer-dependent block so a
    // partition regression reports a clean CHECK failure and still runs cleanup, instead of an
    // out-of-range / null deref crash (mirrors the guard pattern in test_m2exit4).
    if (units.units.size() == 3)
    {
        ...
        if (unit_a != nullptr && unit_d != nullptr)
        {
            // Membership: root unit holds R1; each instance unit holds the child's two entities.
            CHECK(root_unit->entity_indices.size() == 1);
            CHECK(scene.entities[root_unit->entity_indices[0]].id_path ==
                  std::vector<std::string>{kEntR1});
            ...
            // --- 3. independent load / unload isolation (the R-ASSET-005 boundary property)
            {
                ...
                // failure-path control: an unknown id cannot load; unloading a not-resident unit
                // is a no-op.
                ...
                // Unloading A leaves D EXACTLY as it was — the isolation property that lets chunks
