// physics2d fail-closed refusals (M6 P2, R-QA-013 failure paths): the physics2d.* code strings the
...
int main()
{
    // --- the code strings are the exact catalog identities (pins the physics2d.* block) ----------
    CHECK(std::strcmp(kInvalidEntityCode, "physics2d.invalid_entity") == 0);
    CHECK(std::strcmp(kMissingComponentCode, "physics2d.missing_component") == 0);
    CHECK(std::strcmp(kInvalidShapeCode, "physics2d.invalid_shape") == 0);
    CHECK(std::strcmp(kInvalidMassCode, "physics2d.invalid_mass") == 0);
    CHECK(std::strcmp(kInvalidStepCode, "physics2d.invalid_step") == 0);

    kernel::World w;
    PhysicsWorld2d phys;

    // --- invalid shape: non-positive circle radius / box half-extent (world untouched) -----------
    {
        const kernel::Entity e = w.create();
        BodyDesc desc;
        desc.half_extents = {kZero, kOne}; // circle radius 0
        CHECK(same_code(phys.add_body(w, e, desc), kInvalidShapeCode));
        CHECK(!is_body(w, e));

        desc.half_extents = {-kOne, kOne}; // negative radius
        CHECK(same_code(phys.add_body(w, e, desc), kInvalidShapeCode));

        desc.shape = Shape::Box;
        desc.half_extents = {kOne, kZero}; // box with a zero half-extent
        CHECK(same_code(phys.add_body(w, e, desc), kInvalidShapeCode));
        CHECK(!is_body(w, e));
    ...
    // --- invalid entity: null and destroyed handles are refused everywhere -----------------------
    {
        ...
        CHECK(same_code(phys.apply_impulse(w, null_entity, {kOne, kZero}), kInvalidEntityCode));
        ...
        CHECK(same_code(phys.apply_impulse(w, dead, {kOne, kZero}), kInvalidEntityCode));
    }
    ...
    // --- missing component set: physics ops on a live non-body entity ----------------------------
    {
        ...
        CHECK(same_code(phys.apply_impulse(w, plain, {kOne, kZero}), kMissingComponentCode));
        BodyState s;
    ...
    // --- static bodies: impulses are a deterministic no-op ---------------------------------------
    {
        ...
        desc.is_static = true;
        desc.shape = Shape::Box;
        desc.half_extents = {kOne, kOne};
        CHECK(phys.add_body(w, e, desc) == nullptr);
        CHECK(phys.apply_impulse(w, e, {Fixed::from_int(100), kZero}) == nullptr);
        BodyState s;
        CHECK(read_body(w, e, s));
        ...
        dyn.position = {Fixed::from_int(30), kZero}; // far from everything else
        CHECK(phys.add_body(w, d, dyn) == nullptr);
        CHECK(phys.apply_impulse(w, d, {Fixed::from_int(4), kZero}) == nullptr);
