...
## Status — PR 4 of 4 (the tier is COMPLETE)

The #71 work lands as a single-lane 4-PR chain, now COMPLETE. **This directory carries PR 1 (the
wasmtime supply-chain leg, `context_wasm`), PR 3 (the deterministic `WasmRunner`,
`context_wasm_runner` + its `wasm-runner-*` ctests), and PR 4 (the committed `.wasm` fixture corpus
under `fixtures/`, the enumerated fail-closed tests, and the cross-OS determinism gate).**
...
4. **PR 4 — determinism gate + fixtures (LANDED).** The committed, byte-reproducible `.wasm` fixture
   corpus (`fixtures/migrate_hp.wasm` + its `.wat` source; regenerated by
...
WebAssembly binary directly from a readable module description using ONLY the Python stdlib (no
`wat2wasm`/wasmtime needed at regen time — those are unavailable on the local dev host and the
`python-tests` runners), so the committed bytes are byte-identical on every OS and the reproducibility
check runs in the `python-tests` job. The determinism gate executes the committed bytes through the
REAL wasmtime backend in the `wasm-runner` CI job.
